...
name: bweze-cli
description: >-
  Use this skill whenever someone needs a backend, or a task touches BWEZE backend or cloud infrastructure through the BWEZE CLI: projects, SQL, migrations, RLS policies, functions, storage, deployments, compute, secrets, config, schedules, logs, diagnostics, import/export, AI/OpenRouter setup, Stripe/Razorpay payments, Apify web scraping / data sources, PostHog product analytics, backend branches, agent memory (remember/recall project facts and decisions), or CLI docs. For app code with BWEZE or @bweze/sdk, use the bweze app-integration skill instead.
license: Apache-2.0
metadata:
  author: bweze
  version: "1.9.0"
...
Use this skill whenever someone needs a backend, or when managing BWEZE backend and cloud infrastructure with the BWEZE CLI. For application code that calls BWEZE from a frontend, backend, or edge function, use the `bweze` app-integration skill instead.

## Core Rules

- Always run the CLI through `npx @bweze/cli <command>`. Do not install or call a global `bweze` binary.
...
- Treat BWEZE API keys as full-access admin keys. Keep them server-only and out of frontend/public env vars.
...
- At the start of a non-trivial task on a linked project, run `npx @bweze/cli memory list` (cheap, no AI call) and recall any title relevant to the task before designing or debugging. Record decisions and gotchas with `memory remember` at the moment they happen. See `references/memory.md`.
...
| Variable                | Use                                |
| ----------------------- | ---------------------------------- |
| `BWEZE_ACCESS_TOKEN` | Override stored access token       |
| `BWEZE_PROJECT_ID`   | Override linked project ID         |
| `BWEZE_EMAIL`        | Email for non-interactive login    |
| `BWEZE_PASSWORD`     | Password for non-interactive login |

## Connection Setup

If a task needs project access and the connection state is unknown, start with `npx @bweze/cli current`. Use `npx @bweze/cli whoami` when the authenticated identity matters or when `current` reports that the CLI is not authenticated.

If not authenticated, run `npx @bweze/cli login` (opens a browser). For headless / agent / CI contexts with no browser, authenticate non-interactively with a user API key: `npx @bweze/cli login --user-api-key "$BWEZE_USER_API_KEY"` (the user creates the key in the dashboard under Profile → API Keys). If no project is linked, use `npx @bweze/cli link` for an existing project or `npx @bweze/cli create` when the user asked for a new backend. In workflows that are already prelinked or preconfigured, such as CI, local test projects, automation, or explicit user-provided project context, use that project context directly.
...
- Prefer `npx @bweze/cli db migrations new <name>` plus a migration SQL file for schema, grants, indexes, triggers, functions, and RLS policy changes.
- Apply migrations with `npx @bweze/cli db migrations up --all`.
...
- Use `npx @bweze/cli db query <sql>` for targeted inspection and small corrective row/data SQL only when a migration is not appropriate.
- Use `npx @bweze/cli db rpc <fn> [--data <json>]` to call database functions through the backend.
...
- Do not create custom schemas or write to BWEZE-managed/system schemas such as `auth`, `storage`, `realtime`, `payments`, `graphql`, `extensions`, `pg_catalog`, `information_schema`, or `system`, unless you are working on that specific feature module and its docs explicitly allow the operation.
...
- `npx @bweze/cli create` - create a new project. Use `--json` with required flags for non-interactive agent runs. See `references/create.md`.
- `npx @bweze/cli link` - link the current directory to an existing project.
- `npx @bweze/cli current` - show current linked project.
- `npx @bweze/cli metadata --json` - inspect backend metadata when discovery is needed.

Project lifecycle (operates on the linked project unless `--project <id>` is given):

- `npx @bweze/cli projects get [--project <id>]` - show a project's current status, in-flight `operation_status`, region, instance type, and version. Use this to poll after an async operation (restore, version update, instance resize) until `operation_status` clears.
- `npx @bweze/cli projects update [--name <name>] [--domain <domain>] [--storage-size <gib>] [--project <id>]` - rename or change project settings.
...
Org-scoped commands resolve the organization in this order: `--org-id` flag, `BWEZE_ORG_ID`, the linked project's org, the configured default org, then a prompt (or single-org auto-select). Pass `--org-id <id>` to act on a specific org.

- `npx @bweze/cli orgs list` - list organizations you belong to.
- `npx @bweze/cli orgs create <name> [--type personal|team|company]` - create an organization (default type `team`).
- `npx @bweze/cli orgs update [--name <name>] [--type <type>] [--org-id <id>]` - rename or change an organization's type.
- `npx @bweze/cli orgs members list [--org-id <id>]` - list members and pending invitations.
- `npx @bweze/cli orgs members invite <email> [--role administrator|developer] [--org-id <id>]` - invite a member (default role `developer`).
- `npx @bweze/cli orgs members role <memberId> <role> [--org-id <id>]` - change a member's role (`administrator` or `developer`).
- `npx @bweze/cli orgs members remove <memberId> [--org-id <id>]` - remove a member. Confirm intent first.

## Billing and Usage

Inspect the organization's plan/consumption and manage its subscription. Org resolution matches the Organizations section.

- `npx @bweze/cli billing status [--org-id <id>]` - show the current subscription/plan and period.
- `npx @bweze/cli billing credits [--org-id <id>]` - show the credit balance and recent credit transactions.
...
- `npx @bweze/cli backups list [--project <id>]` - list backups.
- `npx @bweze/cli backups latest [--project <id>]` - show the most recent backup.
- `npx @bweze/cli backups create [--name <name>] [--wait] [--project <id>]` - create a backup. `--name` is optional; when provided it must be 1–64 chars. `--wait` blocks until it finishes instead of returning while queued.
- `npx @bweze/cli backups rename <backupId> <name> [--project <id>]` - rename a backup (pass `""` to clear the name).
- `npx @bweze/cli backups delete <backupId> [--project <id>]` - delete a backup. Confirm intent first.
- `npx @bweze/cli backups restore <backupId> [--project <id>]` - restore the project from a backup. This OVERWRITES the project's current database and storage; data written since that backup is lost. Confirm intent first.

## Storage

- `npx @bweze/cli storage buckets` - list buckets.
- `npx @bweze/cli storage create-bucket <name> [--private]` - create a bucket.
- `npx @bweze/cli storage delete-bucket <name>` - delete a bucket and all objects. Confirm destructive intent first.
- `npx @bweze/cli storage list-objects <bucket> [--prefix] [--search] [--limit] [--sort]` - inspect objects.
...
- `npx @bweze/cli functions list` - list deployed functions.
- `npx @bweze/cli functions code <slug>` - view function source.
- `npx @bweze/cli functions deploy <slug> --file <path>` - deploy or update. See `references/functions-deploy.md`.
- `npx @bweze/cli functions invoke <slug> [--data <json>] [--method GET|POST]` - invoke a function.
- `npx @bweze/cli functions delete <slug>` - delete a function. Confirm destructive intent first.

## AI Gateway

- `npx @bweze/cli ai setup` fetches the linked project's active OpenRouter key and writes `OPENROUTER_API_KEY` to a local server-side env file.
...
- `npx @bweze/cli memory list` - cheap title index (no AI call). Run at the start of a non-trivial task; recall any title relevant to the task.
- `npx @bweze/cli memory recall "<query>" [--scope] [--limit] [--threshold]` - semantic + keyword recall.
- `npx @bweze/cli memory remember "<content>" [--kind] [--title] [--scope] [--source]` - store one atomic memory. Record decisions and gotchas at the moment they happen, not at session end.
- `npx @bweze/cli memory remember --file <path>` - extract durable memories from a transcript or notes file.
...
Runtime checkout, subscriptions, customer portal flows, and app code belong in the `bweze` app-integration skill.
...
- Use `npx @bweze/cli deployments deploy <dir>` for frontend source directories. Do not deploy generated output directories unless the deployment reference explicitly calls for it.
- See `references/deployments/deploy.md`.

Custom domains:

- Use `npx @bweze/cli domains ...` for custom domains, Cloudflare Registrar, DNS sync, and SSL verification.
- See `references/deployments/domains.md`.

Backend compute services:

- Use `npx @bweze/cli compute ...`; do not manage BWEZE compute services directly with the user's own `flyctl` account.
...
- `npx @bweze/cli secrets list [--all]` - list secret keys without values.
- `npx @bweze/cli secrets get <key>` - retrieve a secret value only when necessary.
...
- `npx @bweze/cli schedules list/get/create/update/delete/logs`.
...
- Headers can reference BWEZE secrets with `${{secrets.KEY_NAME}}`.
...
- `npx @bweze/cli branch create <name> [--mode full|schema-only] [--no-switch]`
- `npx @bweze/cli branch list`
- `npx @bweze/cli branch switch <name>` or `--parent`
- `npx @bweze/cli branch merge <name> [--dry-run] [--save-sql <path>]`
- `npx @bweze/cli branch reset <name>`
- `npx @bweze/cli branch delete <name>`
...
- `npx @bweze/cli diagnose` - full health report.
- `npx @bweze/cli diagnose --ai "<issue description>"` - ask the BWEZE debug agent to diagnose a concrete backend issue.
- `npx @bweze/cli diagnose metrics [--range 1h|6h|24h|7d]` - EC2 metrics.
- `npx @bweze/cli diagnose advisor [--severity critical|warning|info] [--category security|performance|health]` - advisor issues.
- `npx @bweze/cli diagnose db [--check <checks>]` - database health checks.
- `npx @bweze/cli diagnose logs [--source <name>] [--limit <n>]` - aggregate error logs.
- `npx @bweze/cli logs <source> [--limit <n>]` - source-specific backend logs.

Typical log sources include `function.logs`, `function-deploy.logs`, `postgres.logs`, `postgrest.logs`, and `bweze.logs`. See `references/diagnostics.md` for common debugging scenarios and source selection.

## Documentation

- `npx @bweze/cli docs` - list documentation topics.
- `npx @bweze/cli docs instructions` - setup guide.
- `npx @bweze/cli docs <feature> <language>` - feature docs for `db`, `storage`, `functions`, `auth`, `ai`, or `realtime` in `typescript`, `swift`, `kotlin`, or `rest-api`.

For application code with BWEZE or `@bweze/sdk`, use the `bweze` app-integration skill and use `docs` only as official feature reference.
...
```bash
BWEZE_EMAIL=$EMAIL BWEZE_PASSWORD=$PASSWORD npx @bweze/cli login --email -y
npx @bweze/cli link --project-id $PROJECT_ID --org-id $ORG_ID -y
npx @bweze/cli db query "SELECT 1 AS ok" --json
```
