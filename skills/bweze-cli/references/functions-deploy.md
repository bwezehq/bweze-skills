# npx @bweze/cli functions deploy
...
```bash
npx @bweze/cli functions deploy <slug> [options]
```
...
npx @bweze/cli functions deploy my-handler --file ./my-handler.ts

# Deploy with a display name and description
npx @bweze/cli functions deploy cleanup-expired --file ./handler.ts --name "Cleanup Expired" --description "Removes expired records"

# Update an existing function
npx @bweze/cli functions deploy payment-webhook --file ./webhooks/payment.ts
```
...
- Import `createClient` from `npm:@bweze/sdk`
...
| Mistake | Solution |
|---------|----------|
| Invoking non-existent function | Check functions first with `npx @bweze/cli functions list`, create if needed |
...
2. Deploy                         → npx @bweze/cli functions deploy {slug} --file <path>
3. Check status                   → npx @bweze/cli functions list
4. Ensure secrets are set         → npx @bweze/cli secrets add BWEZE_BASE_URL https://...
5. Invoke from frontend           → bweze.functions.invoke('{slug}', { body: {...} })
```
