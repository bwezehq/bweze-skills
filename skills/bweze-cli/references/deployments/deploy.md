...
```bash
npx @bweze/cli deployments deploy [source-directory] [options]
```
...
npx @bweze/cli deployments deploy . --env '{"VITE_API_URL": "https://my-app.us-east.bweze.app", "VITE_ANON_KEY": "ik_xxx"}'

# JSON output
npx @bweze/cli deployments deploy . --json
```
...
**CRITICAL: Always verify local build succeeds before deploying to BWEZE.**
...
# - Vite: VITE_BWEZE_URL
# - Next.js: NEXT_PUBLIC_BWEZE_URL
# - CRA: REACT_APP_BWEZE_URL
# - Astro: PUBLIC_BWEZE_URL
cat > .env.production << 'EOF'
BWEZE_URL=https://your-project.bweze.app
BWEZE_ANON_KEY=your-anon-key
EOF
...
|-----------|--------|---------|
| Vite | `VITE_` | `VITE_BWEZE_URL` |
| Next.js | `NEXT_PUBLIC_` | `NEXT_PUBLIC_BWEZE_URL` |
| Create React App | `REACT_APP_` | `REACT_APP_BWEZE_URL` |
| Astro | `PUBLIC_` | `PUBLIC_BWEZE_URL` |
| SvelteKit | `PUBLIC_` | `PUBLIC_BWEZE_URL` |
...
Alternatively, pass env vars inline: `npx @bweze/cli deployments deploy . --env '{"VITE_BWEZE_URL": "...", "VITE_BWEZE_ANON_KEY": "..."}'`

### Check Deployment Status

Wait 30 seconds to 1 minute, then check status with `npx @bweze/cli deployments status <id>`.
