...
- `npx @bweze/cli domains cloudflare login` - open Cloudflare OAuth and save the selected Cloudflare account plus OAuth token locally.
...
- `npx @bweze/cli domains search <query> [--limit <n>] [--tlds com,dev]` - search Cloudflare Registrar. `--tlds` is only a local filter; do not assume a fixed TLD allowlist.
- `npx @bweze/cli domains check <domain...>` - check real-time availability and pricing.
- `npx @bweze/cli domains buy <domain>` - register in the connected Cloudflare account. Registration enables auto-renew and WHOIS redaction.
- `npx @bweze/cli domains attach <domain>` - attach to the linked BWEZE deployment.
- `npx @bweze/cli domains dns sync <domain>` - write BWEZE/Vercel DNS records to Cloudflare DNS.
- `npx @bweze/cli domains verify <domain>` - trigger BWEZE custom-domain verification.
- `npx @bweze/cli domains status <domain> [--cloudflare]` - inspect BWEZE status and optionally Cloudflare registration status.
- `npx @bweze/cli domains resume <domain>` - continue attach/DNS/verify after async registration finishes.
- `npx @bweze/cli domains buy-and-attach <domain>` - run register, attach, DNS sync, and verify in one flow.
