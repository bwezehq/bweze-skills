...
```bash
npx @bweze/cli db export [options]
```
...
```bash
# Full export to file
npx @bweze/cli db export --output backup.sql

# Schema only
npx @bweze/cli db export --no-data --output schema.sql

# Specific tables with functions
npx @bweze/cli db export --tables users,posts --include-functions --output partial.sql

# JSON format
npx @bweze/cli db export --format json --output backup.json

# Limited rows for development
npx @bweze/cli db export --row-limit 100 --output dev-data.sql
```
