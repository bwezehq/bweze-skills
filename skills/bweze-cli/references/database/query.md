...
```bash
npx @bweze/cli db query <sql> [options]
```
...
```bash
# Basic query
npx @bweze/cli db query "SELECT * FROM posts LIMIT 10"

# Update rows
npx @bweze/cli db query "UPDATE posts SET status = 'published' WHERE id = 'post_123'"

# Insert rows
npx @bweze/cli db query "INSERT INTO posts (title, status) VALUES ('Hello', 'draft')"

# Delete rows
npx @bweze/cli db query "DELETE FROM posts WHERE archived = true"

# Inspect Postgres system catalog
npx @bweze/cli db query "SELECT table_schema, table_name FROM information_schema.tables WHERE table_schema = 'public'"

# Inspect BWEZE-managed schema data
npx @bweze/cli db query "SELECT * FROM auth.users LIMIT 10"

# JSON output for scripting
npx @bweze/cli db query "SELECT count(*) FROM posts" --json
```
...
- BWEZE-managed/system schemas such as `auth`, `storage`, `realtime`, `payments`, `graphql`, `extensions`, `pg_catalog`, `information_schema`, or `system`: do not write or run DDL unless you are working on that specific feature module and its docs explicitly allow the operation.

Use `npx @bweze/cli db migrations new ...` and `npx @bweze/cli db migrations up ...` for schema changes on `public` application objects.
...
## BWEZE SQL References

When writing SQL for BWEZE, use these built-in references:
...
npx @bweze/cli db query "SELECT id, status FROM posts WHERE status IS NULL"

# Backfill missing row values
npx @bweze/cli db query "UPDATE posts SET status = 'draft' WHERE status IS NULL"
```
