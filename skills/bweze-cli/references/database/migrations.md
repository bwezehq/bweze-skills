# npx @bweze/cli db migrations

Manage developer database migration files for a BWEZE project.

## Commands

```bash
npx @bweze/cli db migrations list
npx @bweze/cli db migrations fetch
npx @bweze/cli db migrations new <migration-name>
npx @bweze/cli db migrations up <migration-file-name-or-version>
npx @bweze/cli db migrations up --to <migration-file-name-or-version>
npx @bweze/cli db migrations up --all
```
...
npx @bweze/cli db migrations new create-posts

# Apply by exact filename
npx @bweze/cli db migrations up 20260418091500_create-posts.sql

# Apply by version
npx @bweze/cli db migrations up 20260418091500

# Apply all pending migrations through a target
npx @bweze/cli db migrations up --to 20260418110000

# Apply all pending migrations
npx @bweze/cli db migrations up --all

# JSON output
npx @bweze/cli db migrations list --json
```
...
1. **Use migrations for schema changes**
   ...
   - `project_admin` can manage and own objects in `public`, but access to BWEZE-managed schemas is restricted.
...
| Mistake | Solution |
|---------|----------|
| Naming files manually with underscores or spaces | Use `npx @bweze/cli db migrations new <migration-name>` |
| Reaching for `db query` to create or alter schema | Use migration files for schema changes; reserve `db query` for row changes |
| Trying to alter BWEZE-managed tables like app-owned tables | Keep generic schema, RLS, trigger, function, and grant changes on `public` application objects; use feature-specific docs for managed module hooks or RLS |
...
```text
1. Check remote history             → npx @bweze/cli db migrations list
2. Sync applied files when useful   → npx @bweze/cli db migrations fetch
3. Create the next migration file   → npx @bweze/cli db migrations new <migration-name>
4. Edit the SQL file                → migrations/<version>_<migration-name>.sql
5. Apply the migration              → npx @bweze/cli db migrations up <filename> or --all
