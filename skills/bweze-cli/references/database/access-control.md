# Database Access Control for BWEZE
...
BWEZE uses three built-in PostgreSQL roles:
...
Raw SQL from `db query` and migration files runs as `project_admin`. This role can manage and own objects in `public`; access to BWEZE-managed schemas is restricted.
...
- BWEZE migrations already run against `public`; schema-qualified references keep helper functions explicit.
...
BWEZE grants broad DML privileges on `public` tables to `anon` and
...
If an operation or field is narrower than BWEZE's broad public-table runtime
