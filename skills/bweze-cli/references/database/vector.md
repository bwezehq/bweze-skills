...
Use this reference when configuring pgvector with the BWEZE CLI: vector
...
with `@bweze/sdk`, use the `bweze` app-integration skill's AI/RAG guidance after this backend
schema is in place.

## Migration Pattern

DDL belongs in a migration. Create a migration file with
`npx @bweze/cli db migrations new <name>`, put SQL like the example below in
that file, then apply it with `npx @bweze/cli db migrations up --all`.
