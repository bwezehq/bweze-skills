...
```bash
npx @bweze/cli diagnose
npx @bweze/cli diagnose --ai "<issue description>"
npx @bweze/cli diagnose metrics --range 24h
npx @bweze/cli diagnose advisor --severity critical
npx @bweze/cli diagnose db --check bloat,slow-queries
npx @bweze/cli diagnose logs --limit 100
```
...
```bash
npx @bweze/cli logs function.logs --limit 50
npx @bweze/cli logs postgres.logs --limit 50
...
| Auth or API error                 | `logs bweze.logs`                                                                   |
| API returning 500 errors          | `logs bweze.logs`, `logs postgrest.logs`                                            |
