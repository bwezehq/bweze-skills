...
1. Reads `.bweze/project.json` from the current directory to find your BWEZE project ID
...
| Mistake | Solution |
|---------|----------|
| Running `npx @bweze/cli posthog setup` outside the linked project directory | The CLI reads `.bweze/project.json` from cwd. Run it from the project root after `npx @bweze/cli link --project-id <id>` |
