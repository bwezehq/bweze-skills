...
> private registry, or report issues, contact the BWEZE team
> (support@bweze.com or your shared Slack channel).

> 🔧 **DO NOT call `flyctl` directly to manage BWEZE compute services.**
> BWEZE runs containers on Fly.io under the hood, but the Fly account, org,
> IPs, and machine ownership all live on the BWEZE cloud. Using `flyctl`
...
# Off-the-shelf image
npx @bweze/cli compute deploy --image nginx:alpine --name proxy --port 80

# Pre-built image from GHCR
npx @bweze/cli compute deploy \
...
npx @bweze/cli compute deploy ./worker \
  --name batch \
  --port 8080 \
...
# Deploy:
$ npx @bweze/cli compute deploy . --name my-bot --port 8080
...
The BWEZE skill knows these patterns; ask the agent and it'll write one.
...
CLI → OSS instance → BWEZE cloud backend → Fly.io. The cloud:
...
- The user never needs to handle a Fly token. The BWEZE cloud holds the org token; per deploy it mints an app-scoped, attenuated token (~20 min, `else: deny`) and the CLI exports it as `FLY_API_TOKEN` only for the duration of the flyctl subprocess.
