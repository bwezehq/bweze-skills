...
2. Server Primitives & Env     → lib/okx-facilitator.ts, lib/x402.ts, lib/bweze.ts, .env
...
| `NEXT_PUBLIC_BWEZE_URL` | BWEZE Dashboard → Project Settings | client + server |
| `NEXT_PUBLIC_BWEZE_ANON_KEY` | BWEZE Dashboard → Project Settings | client |
| `BWEZE_SERVICE_KEY` | BWEZE Dashboard → Project Settings (server-only) | server |
| `OPENROUTER_API_KEY` | BWEZE Dashboard → Model Gateway → Overview | server |
...
```bash
npx @bweze/cli db import migrations/db_init.sql
```

**✓ Verify**

```bash
npx @bweze/cli db query "select count(*) from x402_payments" --json
# → rowCount: 1, count: "0"

npx @bweze/cli db query "select pattern, enabled from realtime.channels where pattern = 'x402_payments'" --json
# → enabled: true

npx @bweze/cli db query "select tgname from pg_trigger where tgrelid = 'x402_payments'::regclass and not tgisinternal" --json
...
export function createServiceClient() {
  return createClient({
    ...
    anonKey: process.env.BWEZE_SERVICE_KEY!, // service key goes through the anonKey slot (Bearer token)
  });
}
...
# BWEZE (dashboard → Project Settings → API Keys)
NEXT_PUBLIC_BWEZE_URL=
...
```bash
node --env-file=.env -e "['OKX_API_KEY','OKX_SECRET_KEY','OKX_PASSPHRASE','PAYMENT_RECIPIENT','NEXT_PUBLIC_BWEZE_URL','NEXT_PUBLIC_BWEZE_ANON_KEY','BWEZE_SERVICE_KEY','OPENROUTER_API_KEY'].forEach(k => console.log(k, process.env[k] ? 'ok' : 'MISSING'))"
# → every row 'ok'
```
...
export async function GET() {
  ...
  const { data: payments, error } = await bweze.database
    .from("x402_payments")
    .select()
  ...
  const { data: allPayments, error: statsError } = await bweze.database
    .from("x402_payments")
    .select("amount");
...
```typescript
// src/lib/bweze-browser.ts
"use client";
import { createBrowserClient } from "@/lib/bweze";
import type { BWEZEClient } from "@bweze/sdk";

let cached: BWEZEClient | null = null;
export function getBrowserClient(): BWEZEClient {
...
import { getBrowserClient } from "@/lib/bweze-browser";
...
export function usePayments() {
  ...
  useEffect(() => {
    ...
    bweze.realtime.on("INSERT_x402_payments", onInsert);

    bweze.realtime.connect()
      .then(() => bweze.realtime.subscribe("x402_payments"))
      .catch(console.error);

    return () => {
      bweze.realtime.off("INSERT_x402_payments", onInsert);
      bweze.realtime.unsubscribe("x402_payments");
    };
  }, [bweze]);
