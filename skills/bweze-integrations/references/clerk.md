...
6. Initialize BWEZE client      → createClient + setAccessToken with Clerk token (refresh on interval)
7. Set up BWEZE database        → requesting_user_id() + table + RLS
8. Build features                  → CRUD pages using BWEZE client
```
...
- Note down **URL** and **Anon Key** from BWEZE, then use the CLI output as the signing key in Clerk
...
export function useBwezeClient(): { client: BWEZEClient; isReady: boolean } {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const token = await getToken({ template: 'bweze' });
        if (cancelled) return;
        ...
        client.setAccessToken(null);
        setIsReady(false);
        console.error('Failed to refresh Clerk token for BWEZE client', err);
      }
    };
...
| `NEXT_PUBLIC_BWEZE_BASE_URL` | BWEZE Dashboard | Exposed to the browser |
| `NEXT_PUBLIC_BWEZE_ANON_KEY` | BWEZE Dashboard | Exposed to the browser |
