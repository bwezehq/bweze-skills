...
# BWEZE + Auth0 Integration Guide

Auth0 signs a BWEZE-compatible JWT inside a **Post Login Action**, embeds it as a custom claim on the ID token, and the Next.js app extracts it to pass to the BWEZE client as `accessToken` (deprecated alias: `edgeFunctionToken`). BWEZE validates the token and uses the `sub` claim for Row Level Security.
...
7. Create BWEZE client utility → lib/bweze.ts
8. Set up BWEZE database        → requesting_user_id() + table + RLS
9. Build features                  → CRUD pages using BWEZE client
```
...
- Name: `Generate BWEZE Token`, trigger: **Post Login**
- Add `jsonwebtoken` as a dependency in the action editor
- Add `BWEZE_JWT_SECRET` in the action's **Secrets** tab
...
- Note down **URL** and **Anon Key** from BWEZE, then store the CLI value in Auth0 as `BWEZE_JWT_SECRET`

## Auth0 Post Login Action

This code runs in Auth0's environment (not your app). The action must sign a JWT with the BWEZE secret and attach it as a namespaced custom claim on the ID token.
...
exports.onExecutePostLogin = async (event, api) => {
  const bwezeToken = jwt.sign(
    ...
    event.secrets.BWEZE_JWT_SECRET,
    { expiresIn: '1h' }
  );

  api.idToken.setCustomClaim('https://bweze.com/bweze_token', bwezeToken);
};
```
...
beforeSessionSaved: async (session, idToken) => {
  if (idToken) {
    ...
    const bwezeToken = payload["https://bweze.com/bweze_token"];
    if (bwezeToken) {
      (session.user ??= {})["https://bweze.com/bweze_token"] = bwezeToken;
    }
  }
  return session;
}
...
- Create a utility at `lib/bweze.ts` that calls `auth0.getSession()`, reads the token from `session.user["https://bweze.com/bweze_token"]`, and passes it as `accessToken` to `createClient`
...
export async function createBWEZEClient() {
  ...
  const bwezeToken = session?.user?.["https://bweze.com/bweze_token"];
