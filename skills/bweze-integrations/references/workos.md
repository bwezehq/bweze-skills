...
# BWEZE + WorkOS Integration Guide

WorkOS AuthKit handles authentication via middleware. On the server, `withAuth()` retrieves the authenticated user, and a JWT is signed with BWEZE's secret containing the user's ID. The token is passed to BWEZE as `accessToken` (deprecated alias: `edgeFunctionToken`).
...
3. Create/link BWEZE project    → npx @bweze/cli create or link
...
7. Create BWEZE client utility  → lib/bweze.ts (server-side JWT signing)
...
- Create a server-side utility at `lib/bweze.ts`
- Use `withAuth()` to get the WorkOS user
- Sign a JWT with `jsonwebtoken` using `BWEZE_JWT_SECRET`
- Required claims: `sub` (from `user.id`), `role: "authenticated"`, `aud: "bweze-api"`
