---
name: bweze-integrations
description: >-
  Use when wiring an external auth provider (Clerk, Auth0, WorkOS, Kinde,
  Stytch, Better Auth) into BWEZE for JWT-based RLS, or when adding the
...
This skill covers integrating **third-party providers** with BWEZE. Currently two categories are supported: **auth providers** (RLS via JWT claims) and **payment facilitators** (x402 HTTP payment protocol). Each provider has its own guide under this directory.
...
2. **JWT is passed to BWEZE** via `accessToken` in `createClient()` (deprecated alias: `edgeFunctionToken`)
3. **BWEZE exposes claims** through `auth.jwt()` in SQL
...
4. **Server records the settled payment** in a BWEZE table with a realtime trigger for live dashboards
