...
Only specific BWEZE-managed tables allow developer RLS changes. Check the relevant module skill or CLI reference before writing policy SQL for a managed table. If that table is listed as allowing RLS changes, normal RLS operations are allowed and should go in migrations.
...
| `permissive` / `restrictive` | Permissive policies OR together; restrictive AND. Most BWEZE projects use permissive. |
