...
```bash
npx @bweze/cli diagnose advisor [--severity critical|warning|info] [--category security|performance|health] [--limit <n>] [--json]
```
...
- **Not available when linked via `--api-key`.** Requires `bweze login` (Platform auth).
...
# Security focus
npx @bweze/cli diagnose advisor --category security

# Performance focus (often pairs with db-health to verify)
npx @bweze/cli diagnose advisor --category performance
npx @bweze/cli diagnose db --check slow-queries,index-usage
