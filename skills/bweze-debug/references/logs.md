...
```bash
npx @bweze/cli logs <source> [--limit <n>]
```
...
|--------|------------------|----------------|
| `bweze.logs` | Main backend (auth, API gateway, realtime, edge function dispatcher, deploy controller) | Auth/OAuth errors, realtime WS errors, generic 5xx, signup/login failures |
...
```bash
npx @bweze/cli diagnose logs [--source <name>] [--limit <n>]
```
...
npx @bweze/cli logs postgREST.logs --limit 100

# If postgREST log shows "SQL error", drop into postgres
npx @bweze/cli logs postgres.logs --limit 100
```
