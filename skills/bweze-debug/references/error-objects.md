...
| `AUTH_*` / OAuth errors | Backend auth | [logs](logs.md) `bweze.logs` |
...
| **401** | Auth token missing / invalid / expired | [logs](logs.md) `bweze.logs` + [metadata](metadata.md) auth config |
...
| `/api/auth/...` | Auth backend | [logs](logs.md) `bweze.logs` |
| `/api/storage/...` | Storage | [logs](logs.md) `bweze.logs` |
...
2. Read `message` / `details` to identify the offending column name. Verify the table schema with `npx @bweze/cli db tables` or `npx @bweze/cli metadata --json`.
