...
- `endpoint` = `https://your-appkey.your-region.bweze.app/storage/v1/s3`
- `region` = `us-east-2`
...
[profile bweze]
region = us-east-2
endpoint_url = https://your-appkey.your-region.bweze.app/storage/v1/s3
s3 =
  addressing_style = path
```

```bash
aws --profile bweze s3 ls
aws --profile bweze s3 cp ./photo.jpg s3://my-bucket/photo.jpg
aws --profile bweze s3 sync ./dist s3://my-bucket/dist
```
...
```bash
rclone copy ./dist bweze:my-bucket/dist
rclone sync bweze:my-bucket/backups ./local-backups
```
...
This means buckets created via `npx @bweze/cli storage create-bucket` are reachable over S3, and objects uploaded with `aws s3 cp` can be served via the SDK's `download()` using the same key.
...
3. Verify connectivity       → aws --profile bweze s3 ls
