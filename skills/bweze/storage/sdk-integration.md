...
Use `createBrowserClient()` for authenticated browser uploads. It reads the browser-readable `bweze_access_token` cookie and refreshes through `/api/auth/refresh`; the refresh token remains httpOnly.
...
const { data: post } = await bweze.database
  .from('posts')
  .select('image_key')
  .eq('id', 'post-123')
  .single()
...
// Download using key
const { data: blob, error } = await bweze.storage
  .from('images')
...
- In Next.js / SSR apps, direct browser uploads should use `createBrowserClient()` from `@bweze/sdk/ssr` so Storage RLS sees the signed-in user
...
1. **Verify bucket exists before uploading**
   - Check available buckets via CLI: `bweze storage buckets`
...
```
1. Check available buckets → bweze storage buckets
