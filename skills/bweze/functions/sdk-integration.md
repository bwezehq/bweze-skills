...
Use BWEZE SDK to invoke serverless functions from your frontend application.
...
const { data, error } = await bweze.functions.invoke('hello-world', {
  body: { name: 'World' }
})

// GET request
const { data, error } = await bweze.functions.invoke('get-stats', {
  method: 'GET'
})

// With custom headers
const { data, error } = await bweze.functions.invoke('api-endpoint', {
  method: 'PUT',
...
1. **Verify function exists and is active before invoking**
   - Check available functions via CLI: `bweze functions list`
...
```
1. Check available functions → bweze functions list
