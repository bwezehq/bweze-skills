...
Ensure your `.env` is configured with your BWEZE URL and anon key. Get the anon key with `npx @bweze/cli secrets get ANON_KEY`. See the main [SKILL.md](../SKILL.md) for framework-specific variable names.
...
```javascript
const { data, error } = await bweze.emails.send({
  to: 'user@example.com',
...
```bash
curl -X POST "$BWEZE_URL/api/email/send-raw" \
  -H "Authorization: Bearer $BWEZE_ANON_KEY" \
...
| Task | Code |
|------|------|
| Send to one recipient | `bweze.emails.send({ to, subject, html })` |
| Send to many | `bweze.emails.send({ to: [a, b], cc, bcc, subject, html })` |
