...
then store durable outputs in BWEZE Storage.
...
Start with [overview.md](overview.md): run `npx @bweze/cli ai setup` and keep
...
permanent asset store. Upload the generated file to BWEZE Storage and save
...
await bweze.database.from('generated_images').insert([{
...
3. Persist generated images in BWEZE Storage before storing database rows.
