...
BWEZE Postgres with pgvector. BWEZE remains the database/vector store;
...
- [BWEZE CLI pgvector setup](../../bweze-cli/references/database/vector.md)

## Setup

Bring up the `vector` extension, vector tables, indexes, and match RPCs through
a BWEZE CLI database migration; see
[database/vector.md](../../bweze-cli/references/database/vector.md).
...
Use BWEZE for the durable content row and vector. Keep one embedding model
per vector column.
...
async function storeDocument(content: string) {
  ...
  const { data, error } = await bweze.database.from('documents').insert([{
    content,
...
async function askQuestion(sessionId: string, question: string) {
  ...
  const { data: documents, error: searchError } = await bweze.database.rpc('match_documents', {
  ...
  const { error: insertError } = await bweze.database.from('chat_messages').insert([
...
1. Design the BWEZE tables first: source documents, vector column dimension,
