...
wire audio features into BWEZE Storage, database rows, and server-side app
flows.
...
Name the BWEZE guide `audio.md` because it covers both STT and TTS.
Use "transcription" only for STT-specific tables, jobs, or UI.

## Speech-to-Text

For STT, upload the original audio file to BWEZE Storage first, then
...
as audio bytes. Upload the result to BWEZE Storage and save:
...
2. Store audio files in BWEZE Storage and database rows with `url` plus `key`.
