...
Use OpenRouter as the model gateway for new BWEZE AI features. BWEZE
provides the project-level OpenRouter key; app code should call OpenRouter from
server-side code and use BWEZE for data, storage, auth, and RLS.
...
```bash
npx @bweze/cli ai setup
```

This fetches the active OpenRouter key from the linked BWEZE backend and
...
unavailable, ask the user to upgrade `@bweze/cli` or copy the key from the
BWEZE dashboard Model Gateway.
...
| [image-generation.md](image-generation.md) | Generate or edit images, then upload outputs to BWEZE Storage |
...
| [embeddings-and-rag.md](embeddings-and-rag.md) | Generate embeddings, store/search vectors in BWEZE pgvector, and build RAG |
| [models-list.md](models-list.md) | Discover model IDs, modalities, pricing, limits, and dimensions |

## BWEZE Patterns

1. Run `npx @bweze/cli ai setup` before writing OpenRouter code.
2. Store generated files in BWEZE Storage; save both `url` and `key` in the
   database.
3. Store embeddings in Postgres `vector(N)` columns; `N` must match the model's
   output dimension.
4. Use `@bweze/sdk` for database, auth, storage, realtime, and payments.
...
| Using `bweze.ai` for new features | Use OpenRouter APIs with the project OpenRouter key |
| Saving generated base64/media directly in Postgres | Upload to BWEZE Storage and save `url` plus `key` |
