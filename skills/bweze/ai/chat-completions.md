...
classification, and structured text. Use BWEZE for sessions, messages,
...
Start with [overview.md](overview.md): run `npx @bweze/cli ai setup`, keep
`OPENROUTER_API_KEY` server-side, and initialize the OpenAI SDK with
`baseURL: 'https://openrouter.ai/api/v1'`.

## Store a Response

For a chatbot app, keep session/message rows in BWEZE and protect them with
...
never trust a browser-supplied `user_id`. BWEZE database inserts use array
format.
...
async function sendChatMessage(sessionId: string, content: string) {
  const { data: userData, error: userError } = await bweze.auth.getCurrentUser()
  if (userError || !userData?.user?.id) {
    throw userError ?? new Error('Authentication required')
  }

  const history = await bweze.database
    .from('chat_messages')
    .select('role, content')
  ...
  const { error: userInsertError } = await bweze.database
    .from('chat_messages')
    .insert([{
  ...
  const { data, error: assistantInsertError } = await bweze.database
    .from('chat_messages')
    .insert([{
      session_id: sessionId,
      user_id: userData.user.id,
...
1. Store chat sessions and messages in BWEZE, not local browser state, when
...
5. Use BWEZE Realtime to fan out newly inserted assistant messages to other
...
| Keeping chatbot history only in React state | Store sessions/messages in BWEZE and secure them with RLS |
...
| Treating OpenAI SDK errors like BWEZE `{ data, error }` results | OpenAI SDK calls throw; catch exceptions separately |
