...
Use BWEZE SDK to perform CRUD operations in your frontend application.
...
```javascript
// All records
const { data, error } = await bweze.database.from('posts').select()

// Specific columns
const { data } = await bweze.database.from('posts').select('id, title')

// With relationships
const { data } = await bweze.database.from('posts').select('*, comments(id, content)')
```
...
// Bulk insert
const { data } = await bweze.database
  .from('posts')
...
```javascript
const { error } = await bweze.database
  .from('posts')
  .delete()
  .eq('id', postId)
```
...
```javascript
const { data, error } = await bweze.database.rpc('get_user_stats', { user_id: '123' })
```
...
const { error: insertError } = await bweze.database
  .schema('analytics')
  .from('events')
  .insert([{ name: 'signup' }])

const { data: rollup, error: rpcError } = await bweze.database
  .schema('analytics')
...
const { data, count } = await bweze.database
  .from('posts')
...
When creating tables via `bweze db query` (CLI), use these built-in references:
...
-- Policies (see the bweze-cli database access-control reference for advanced patterns)
...
> For access-control best practices (RLS, infinite recursion prevention, SECURITY DEFINER, performance), see the **bweze-cli** skill's [database/access-control.md](../../bweze-cli/references/database/access-control.md) reference.
...
1. **Generate TypeScript interfaces for every table schema**
   - Use `bweze db tables` and `bweze db query` (CLI) to inspect the table schema
...
```typescript
// After checking table schema via `bweze db tables`
...
```
1. Check table schema     → bweze db tables / bweze db query
