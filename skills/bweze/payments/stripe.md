...
Use this guide when writing app code for Stripe payments through `@bweze/sdk`. For CLI setup, catalog sync, webhook setup, and fulfillment migrations, use the `bweze-cli` payments references first.

## Mental Model

- Stripe uses Products and Prices. A subscription is still created through Checkout using recurring Prices.
- BWEZE SDK creates Stripe Checkout Sessions and Billing Portal Sessions.
...
Checkout and portal creation use the caller's BWEZE token. If the app exposes billing subjects such as teams or organizations, add RLS before wiring UI.
...
```typescript
const { data: order, error: orderError } = await bweze.database
  .from('orders')
...
const { data, error } = await bweze.payments.stripe.createCheckoutSession('test', {
  mode: 'payment',
...
```typescript
const { data, error } =
  await bweze.payments.stripe.createCustomerPortalSession('test', {
    subject: { type: 'team', id: teamId },
...
Stripe webhooks are managed by BWEZE when the backend has a public URL:

```bash
npx @bweze/cli payments stripe webhooks configure --environment test
```

BWEZE's Stripe managed event set:
  ...
  IF NEW.provider = 'stripe'
    ...
    v_subject_type := COALESCE(
      NEW.payload -> 'data' -> 'object' -> 'parent'
        -> 'subscription_details' -> 'metadata' ->> 'bweze_subject_type',
      NEW.payload -> 'data' -> 'object' -> 'metadata' ->> 'bweze_subject_type'
    );
    v_subject_id := COALESCE(
      NEW.payload -> 'data' -> 'object' -> 'parent'
        -> 'subscription_details' -> 'metadata' ->> 'bweze_subject_id',
      NEW.payload -> 'data' -> 'object' -> 'metadata' ->> 'bweze_subject_id'
    );
...
| Mistake | Fix |
|---------|-----|
| Calling payment methods at the root `payments` namespace | Use `bweze.payments.stripe.*` |
...
| Resolving subscription subjects only via `payments.customer_mappings` | Read `bweze_subject_*` from the event payload first; events from other types may not have created the mapping yet |
