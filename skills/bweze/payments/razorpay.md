...
Use this guide when writing app code for Razorpay payments through `@bweze/sdk`. For CLI setup, catalog sync, manual webhook instructions, and fulfillment migrations, use the `bweze-cli` payments references first.
...
- Razorpay Checkout runs inside the app; BWEZE returns `checkoutOptions`, not a hosted redirect URL.
...
Razorpay runtime routes use the caller's BWEZE token for authorization probes.
...
const { data, error } = await bweze.payments.razorpay.createOrder('test', {
  amount: 50000,
  currency: 'INR',
...
const checkout = new Razorpay({
  ...
  handler: async (response) => {
    const verified = await bweze.payments.razorpay.verifyOrder('test', {
...
```typescript
const { data, error } = await bweze.payments.razorpay.createSubscription('test', {
  planId: 'plan_123',
  totalCount: 12,
  subject: { type: 'team', id: teamId },
  customerName: user.name ?? null,
...
const checkout = new Razorpay({
  ...data.checkoutOptions,
  handler: async (response) => {
    const verified = await bweze.payments.razorpay.verifySubscription('test', {
...
```typescript
await bweze.payments.razorpay.cancelSubscription('test', 'sub_123', {
  cancelAtCycleEnd: false
})

await bweze.payments.razorpay.pauseSubscription('test', 'sub_123')
await bweze.payments.razorpay.resumeSubscription('test', 'sub_123')
```
...
Razorpay webhook setup is manual. From the BWEZE dashboard (Dashboard -> Payments -> Settings -> Webhooks), copy:

- Webhook URL, for example `/api/webhooks/razorpay/test`
- Webhook secret

Then create a webhook in the Razorpay Dashboard with that URL and secret, and select the events BWEZE handles.

Razorpay can only deliver webhooks to a public HTTPS URL.

Events BWEZE handles:
